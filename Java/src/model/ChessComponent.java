...
//        ChessGameFrame.
    ...
    public List<ChessboardPoint> canMoveToCollect(Chessboard chessboard) {
        ArrayList<ChessboardPoint> arr=new ArrayList<>();
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                ChessComponent chessComponent=chessboard.getChessComponents()[i][j];
                if (this.canMoveTo(chessboard.getChessComponents(),chessComponent.getChessboardPoint())) {
                    arr.add(chessComponent.getChessboardPoint());
                }
            }
        }
        return arr;

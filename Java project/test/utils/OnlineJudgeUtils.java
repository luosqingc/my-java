...
public class OnlineJudgeUtils {
    ...
    public static boolean findChessColorField(){
        Class <?> Enum = findChessColorClass();
        String[] results = {"BLACK", "WHITE", "NONE"};
        if(Enum.isEnum()){
            Object[] Enums = Enum.getEnumConstants();
            if(Enums.length!=results.length) {
                fail("The number of members in the 'ChessColor' is incorrect.Please check the number of members in 'ChessColor'");
                return false;
            }
            for(int i=0;i<results.length;i++){
                if(!Enums[i].toString().equals(results[i])){
                    fail("The "+i+"th member is incorrect. Please check the "+i+"th member in 'ChessColor'");
                    return false;
                }
